...
const Interview = () => {
  const {report,getReportById,loading,setReport}=useInterview(undefined)
            ...
            {report.technicalQuestions.map((q, index) => (
              <div key={index} className="qa-card">
                ...
                <div className="qa-content">
                  <h3>{q.question}</h3>
                  <div className="intention"><strong>Intention:</strong> {q.intention}</div>
                  <div className="answer"><strong>Ideal Answer:</strong> {q.answer}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="interview-layout">
        ...
        <aside className="right-sidebar">
          <div className="match-score-section">
            ...
            <div className={`score-circle ${report.matchScore >= 70 ? 'high' : report.matchScore >= 50 ? 'medium' : 'low'}`}>
            ...
            <p className="score-text">Based on your resume and the job description.</p>
          </div>

          <h3 className="section-title">Skill Gaps</h3>
          <div className="tags-container">
            {report.skillGaps.map((gap, idx) => {
              const severityClass = gap.severity ? `severity-${gap.severity}` : "";
              return (
                <span key={idx} className={`skill-tag ${severityClass}`} title={gap.skill}>
                  {gap.skill}
                </span>
              );
            })}
